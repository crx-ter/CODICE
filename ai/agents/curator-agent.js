const RAG=(()=>{
  ...
  function _expandQuery(rawQuery) {
    ...
    try {
      const ac = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.activeClass : null;
      const ab = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.activeBlock : null;
      const lr = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.lastReferencedClass : null;
      const lb = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.lastReferencedBlock : null;
  ...
  function _classContextScore(chunk, docs) {
    try {
      const ac = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.activeClass : null;
      const lr = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.lastReferencedClass : null;
      const ad = (typeof AI_CONTEXT !== 'undefined') ? AI_CONTEXT.activeDivision : null;
...
async function extractPDF(file,onP){
  if(typeof pdfjsLib==='undefined')throw new Error('PDF.js no cargó. Espera un momento.');
  ...
  for(let i=1;i<=pdf.numPages;i++){
    ...
    if(onP)onP(Math.round(i/pdf.numPages*50),`Página ${i}/${pdf.numPages}`);
  }
  return text;
}
async function extractOCR(file,onP){
  if(typeof Tesseract==='undefined')throw new Error('Tesseract no cargó.');
  const r=await Tesseract.recognize(file,'spa+eng',{
    logger:m=>{if(m.status==='recognizing text'&&onP)onP(Math.round(m.progress*50),'OCR…');}
  });
  return r.data.text;
}
function showIngestOverlay(title){
  $('ingest-title').textContent=title;$('ingest-sub').textContent='Procesando…';
  $('ingest-bar').style.width='0%';$('ingest-pct').textContent='0%';
  $('ingest-detail').textContent='Iniciando…';$('ingest-overlay').classList.remove('hidden');
}
function updateIngest(pct,detail){
  const p=clamp(pct,0,100);
  $('ingest-bar').style.width=p+'%';$('ingest-pct').textContent=p+'%';
  $('ingest-detail').textContent=detail||'';
}
function hideIngestOverlay(){$('ingest-overlay').classList.add('hidden');}

async function ragIngestFiles(e){
  const files=Array.from(e.target?.files||[]);
  if(!files.length)return;
  if(e.target?.value!==undefined)e.target.value='';
  for(const file of files){
    showIngestOverlay('Procesando: '+file.name);
    try{
      let text='';const ext=file.name.split('.').pop().toLowerCase();
      if(ext==='pdf')text=await extractPDF(file,(p,d)=>updateIngest(p*.5,d));
      else if(['png','jpg','jpeg','webp','bmp'].includes(ext))text=await extractOCR(file,(p,d)=>updateIngest(p*.5,d));
      else{text=await file.text();updateIngest(50,'Texto extraído');}
      if(!text.trim()){Toast.error('Sin texto extraíble: '+file.name);hideIngestOverlay();continue;}
      updateIngest(50,'Indexando…');
      await RAG.ingest(file.name,text,ext,(p,cur,tot)=>updateIngest(50+p*.5,`Fragmento ${cur}/${tot}`));
      updateIngest(100,'¡Listo!');await delay(500);Toast.success(`"${file.name}" indexado`);
    }catch(err){Toast.error(err.message);}
    hideIngestOverlay();
  }
  renderRagPanel();
}
async function ragIngestPaste(){
  const name=$('rag-paste-name').value.trim()||'Texto '+new Date().toLocaleDateString('es');
  const text=$('rag-paste-text').value.trim();
  if(!text)return Toast.error('Escribe o pega texto primero');
  closeModal('modal-rag-paste');
  showIngestOverlay('Indexando: '+name);
  try{
    await RAG.ingest(name,text,'text',(p,cur,tot)=>updateIngest(p,`Fragmento ${cur}/${tot}`));
    updateIngest(100,'¡Listo!');await delay(500);Toast.success(`"${name}" indexado`);
  }catch(err){Toast.error(err.message);}
  hideIngestOverlay();renderRagPanel();
}
function openRagPasteModal(){$('rag-paste-name').value='';$('rag-paste-text').value='';openModal('modal-rag-paste');}
const _ragSearchD=debounce(async q=>{
  if(!q.trim()){$('rag-results').innerHTML='';return;}
  renderRagResults(await RAG.search(q,6),q);
},320);
function ragSearch(q){_ragSearchD(q);}
